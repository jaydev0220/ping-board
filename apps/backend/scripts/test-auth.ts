...
const displayResponse = (status: number, data: unknown, setCookieHeader?: string | null): void => {
...
const testRefresh = async (): Promise<void> => {
  ...
  if (lastExtractedToken) {
    console.log(`Last extracted token: ${lastExtractedToken.substring(0, 20)}...`);
  }
  const tokenInput = await prompt("Enter refresh token (or 'auto' to use last extracted): ");
  const token = tokenInput === 'auto' ? lastExtractedToken : tokenInput;

  if (!token) {
    console.error('❌ No refresh token provided and no token extracted from previous requests.');
    return;
  }
