...
async function testTokenRotation() {
	...
	console.log(
		`   📝 Access token: ${loginData.accessToken.substring(0, 20)}...`
	);
	...
	console.log(
		`   📝 New access token: ${firstRefreshData.accessToken.substring(0, 20)}...`
	);
	console.log(`   🍪 New refresh token: ${newToken.substring(0, 20)}...`);
	console.log(
		`   🔍 Full Set-Cookie: ${firstRefreshSetCookie.substring(0, 100)}...`
	);
	console.log(
		`   🔄 Token rotated: ${originalToken !== newToken ? 'YES' : 'NO'}`
	);
	...
	console.log(
		`   📝 New access token: ${secondRefreshData.accessToken.substring(0, 20)}...`
	);
	...
	if (oldTokenRefreshResponse.status !== 401) {
		throw new Error(
			`Expected 401 status but got ${oldTokenRefreshResponse.status}`
		);
	}
	...
	try {
		...
		if (row.consumed_at === null) {
			throw new Error(
				'Token consumed_at is NULL! Token should be marked as consumed.'
			);
		}
